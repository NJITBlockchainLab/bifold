...
const ScanBLE: React.FC<ScanProps> = ({ navigation, route }) => {
  ...
  const bleAdvertiseEmitter = new NativeEventEmitter(NativeModules.BLEPeripheralManagerBridge)
  ...
  useEffect(() => {
    ...
    // const readListener = bleManagerEmitter.addListener('BleManagerDidUpdateValueForCharacteristic', handleRead)
    const readListener = bleAdvertiseEmitter.addListener('onDataReceived', handleRead)
    console.log("Added read listener")
  ...
  const connectToDevice = (deviceId: string) => {
    ...
    BleManager.connect(deviceId)
      .then(async () => {
        ...
        console.log(await BleManager.retrieveServices(deviceId))
  ...
  const startAdvertising = async () => {
    ...
    if (permissionsGranted) {
      try {
        ...
        BLEPeripheralManager.startBLEAdvertising()
        ...
        // BleAdvertise.broadcast(uuid, major, mino)
          // .then((success: any) => {
          //   console.log(success)
          // })
          // .catch((error: string) => {
  ...
  const stopAdvertising = async () => {
    ...
    try {
      // await BLEPeripheralManager.stopBroadcast()
